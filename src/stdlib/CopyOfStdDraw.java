 ...
 */
public final class CopyOfStdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static CopyOfStdDraw std = new CopyOfStdDraw();
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            final URL url = CopyOfStdDraw.class.getResource(filename);
    ...
    public void actionPerformed(ActionEvent e) {
        final FileDialog chooser = new FileDialog(CopyOfStdDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        final String filename = chooser.getFile();
        if (filename != null) {
            CopyOfStdDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = CopyOfStdDraw.userX(e.getX());
            mouseY = CopyOfStdDraw.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    public static void main(String[] args) {
        CopyOfStdDraw.square(.2, .8, .1);
        CopyOfStdDraw.filledSquare(.8, .8, .2);
        CopyOfStdDraw.circle(.8, .2, .2);

        CopyOfStdDraw.setPenColor(CopyOfStdDraw.BOOK_RED);
        CopyOfStdDraw.setPenRadius(.02);
        CopyOfStdDraw.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        CopyOfStdDraw.setPenRadius();
        CopyOfStdDraw.setPenColor(CopyOfStdDraw.BOOK_BLUE);
        final double[] x = { .1, .2, .3, .2 };
        final double[] y = { .2, .3, .2, .1 };
        CopyOfStdDraw.filledPolygon(x, y);

        // text
        CopyOfStdDraw.setPenColor(CopyOfStdDraw.BLACK);
        CopyOfStdDraw.text(0.2, 0.5, "black text");
        CopyOfStdDraw.setPenColor(CopyOfStdDraw.WHITE);
        CopyOfStdDraw.text(0.8, 0.8, "white text");
    }
